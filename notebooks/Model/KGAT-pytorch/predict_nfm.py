...
def predict(args):
    ...
    if args.model_type == "fm":
        if args.use_user_info == 0:
            np.save(args.save_dir + 'cf_scores_fm0.npy', cf_scores)
        else:
            np.save(args.save_dir + 'cf_scores_fm1.npy', cf_scores)
    else:
        if args.use_user_info == 0:
            np.save(args.save_dir + 'cf_scores_nfm0.npy', cf_scores)
        else:
            np.save(args.save_dir + 'cf_scores_nfm1.npy', cf_scores)
