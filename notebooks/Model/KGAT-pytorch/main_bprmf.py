...
def save_embeddings(model, save_dir):
    # Extract item and user embeddings from the trained model
    user_embeddings = model.user_embed.weight.data.cpu().numpy()
    item_embeddings = model.item_embed.weight.data.cpu().numpy()
    ...
    np.savez(os.path.join(save_dir, 'mf.npz'), user_embed=user_embeddings, item_embed=item_embeddings)

    print(f"Embeddings saved to {os.path.join(save_dir, 'mf.npz')}")
...
def train(args):
    ...
    save_embeddings(model, args.save_dir)
